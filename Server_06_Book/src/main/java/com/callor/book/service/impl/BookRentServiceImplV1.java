...
public class BookRentServiceImplV1 implements BookRentService{
	...
	/*exception 처리방법 2가지
	 * 1.try-catch를 사용하기
	 * exception이 발생할 코드를 감싸서 직접 처리하기
	 * 2. 떠넘기기 (던지기)
	 * method throws를 추가하여 호출한 method 에게 exception을 되돌려 보내기
	...
	public BookRentDTO findById(Long seq) {
		 ...
		 * "WHERE 도서명="+title;
		 * title변수값에 1 OR 1=1 과 같은 문자열을 담아서 보내면
		 * 실제 쿼리는 WHERE 도서명=1 OR 1=1 과 같이 만들어져버린다 
		 * PrepareStatement 를 사용할때는 조건문의 변수가 사용될 위치에
		 * ? 기호를 사용한다
		 * WHERE 도서명 =? 와 같이 사용함
		 */
		String sql="SELECT * FROM view_도서대여정보";
		...
		try {
			//아래의 2개 코드에 의해서 where주문번호=seq값 형식의 SQL이 만들어진다
			pStr=dbConn.prepareStatement(sql);
			pStr.setLong(1, seq);
...
//			rSet.close();
	...
	public List<BookRentDTO> findByBISBN(String isbn) {
		...
		try {
			...
			List<BookRentDTO>brList=this.selectAll();
			pStr.close();
			return brList;
