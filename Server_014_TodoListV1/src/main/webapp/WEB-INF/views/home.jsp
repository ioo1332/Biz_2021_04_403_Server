...
table.td_list {
	...
	/* table을 그릴때 각td와 td간격에 미세하게 여백이 있어서 가끔 선을그리면
	선과 선사이에 틈새가 보인다 이러한 틈새를 막기위해 설정하는값
	*/
	border-spacing: 0;
}
...
table.td_list td.doit {
	...
	*/
	max-width: 80%;
	/* 
	  td의 width 보다 큰text를 말줄임표로 표현하기
	  td 3개인데
	  첫번째 세번째 td 20%설정했기때문에
	  여기 td는 전체 table width 를 으로 지정하고 ellipsis로 설정해주면된다
	*/
	max-width: 0;
	overflow: hidden;
...
/* media query 
	반응형 web에서 사용하는 css기법
	max-width : 화면 폭이 이 값보다 작으면
	min-width : 화면이 값보다 크면
...
<script>
/* script가 어디에 위치하든 상관없이 이후의 
 * 
 * 다른 event가 정상적으로 설정될수 있도록 선언
...
document.addEventListener("DOMContentLoaded",()=>{
	...
	//function(){ }
	 document.querySelector("table.td_list").addEventListener("dblclick",(ev)=>{
		 ...
		 if(tagName=="TD"){
			 //방금 더블클릭된 TD를 감싸고 있는 TR에서 데이터를 가져와라
...
<body>
	...
	<%-- form tag의 action속성
		form tag의 action속성은 form에 담긴 데이터를 submit할때(서버로 전송할때)
		어떤 uri path를 통해서 서버에 보낼것인가를 지정하는것
		만약 이 action속성을 지정하지 않으면 현재 파일(home.jsp)을 보여줄때 사용한
		uri주소가 자동으로 설정이 된다
		
		${rootPath}/처럼 주소를 지정하는것
		form, a tag등에 URL,URI를 지정할때
		주소의 지정방식에 따라 상대주소,절대주소 방법이 있는데 지정하는방법에따라
		연결이 잘 안되는 경우가 많다
		우리 프로젝트는 모두 절대주소 지정방식으로 통일하고 항상 주소(URL,URI)와 
		관련된 모든 항목은 ${rootPaht}시작하는 주소로 사용한다
