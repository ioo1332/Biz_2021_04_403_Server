...
// localhost:8080/db/book(uri) 이라고 요청을하면 
...
public class BookController extends HttpServlet{
	...
	/* WAS 관련 프로젝트를 수행할때 클래스를 만들고 서버를 한번 run한후에
	 * 다시 클래스 코드를 변경하면 Tomcat이 자동으로 재시작된다
	 * 그떄 Tomcat이 내부적으로 클래스의 정보를 참조하는 일종의 Key값
	 * 
	 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * *부분에 추가된 subpath를 추출하여  여러가지 요청을 처리할수있다
	...
	public void callService() {
		...
		List<BookVO>bookList=bService.selectAll();
		//도서정보 조회
		BookVO reVO =bService.findById("9700000");
		//도서정보 변경
